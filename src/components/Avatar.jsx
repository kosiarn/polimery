...
const Avatar = ({ avatarLink, personnelPageLink, name, title1, title2, showNameCard, role }) => {
	const AvatarDiv = (
		<div 
		...
		>
			<div className={`w-full md:min-w-[120px] max-h-[64px] md:max-h-none py-[8px] md:py-[16px] flex flex-col justify-center items-center bg-black/75 ${showNameCard ? "visible" : "invisible"}`}>
				<p className="text-white font-regular text-[10px] md:text-[16px]">{title1}</p>
				<p className="text-white font-regular text-[10px] md:text-[16px] w-max-32 md:w-max64">{name}{title2 ? "," : ""}</p>
				{ title2 && <p className="text-white font-regular text-[10px] md:text-[16px] w-max-32 md:w-max64">{title2}</p>}
